...
A pairs trading implementation built around a simple mean reversion strategy, using **Coke (KO)** and **Pepsi (PEP)** as the test pair. The core question: can a volatility-of-volatility approach outperform the standard Bollinger Band method?
...
| `SAVE_*.py` | Scripts used to save data locally — kept for documentation clarity |
...
### 2025
A different story. The deviation strategy finished at approximately **+12% total return**. The Bollinger approach failed to capture the relationship and returned roughly **-2% PnL**. Notably, the deviation strategy outperformed *despite* starting later in the year due to its burn-in requirement.
...
BBG uses the **Polars** dataframe library, which is broadly compatible with the Python ecosystem. Key nuances worth knowing:

- Data comes in a vertical format and needs to be pivoted to horizontal (wide) format for this use case
- Mutation functions are clearly named — `rolling_mean` does exactly what it says — which made the learning curve manageable

---

## Known Constraints

**Fixed start/end windows** — Both out-of-sample tests don't begin trading until approximately June of each respective year, cutting the opportunity set roughly in half. Even with this limitation, the deviation strategy still outperformed the Bollinger approach in 2025.
...
- Rolling or expanding window approach to eliminate the hard start-date limitation
