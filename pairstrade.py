...
#converting from pandas df to polars df to run my strategy
df_wide = pl.from_pandas(df_pandas)

#once i started using the csv instead of the raw BBG pull, I didn't need to reorganize the DF from vertical to horizontal
'''
...
def run_strategy_deviation(df_wide, window, k_mult):
    ...
    df_price_deviation = df_wide.with_columns(
    ...
    df_ratio_deviation = df_price_deviation.with_columns(
    ...
    df_error_deviation = df_ratio_deviation.with_columns(
    ...
    df_noise_deviation = df_error_deviation.with_columns(
        (pl.col('Deviation').rolling_std(window_size=window).abs().alias('Moving 30 Day Price Ratio Std Deviation'),
        pl.col('date').cast(pl.Date)))
    #print(df_noise)
    ...
    df_boundary_deviation = df_noise_deviation.with_columns(
        ((pl.col('Moving 30 Day Price Ratio Avg') + (pl.col('Moving 30 Day Price Ratio Std Deviation') * k_mult)).alias('Upper Bound')),
        (((pl.col("Moving 30 Day Price Ratio Avg")) - (pl.col('Moving 30 Day Price Ratio Std Deviation') * k_mult)).alias('Lower Bound')),
        pl.col('date').cast(pl.Date))
    #print(df_boundary)
    ...
    df_conditional_deviation = df_boundary_deviation.with_columns(
    ...
    df_pnl_deviation = df_conditional_deviation.with_columns(
    ...
    daily_returns = df_pnl_deviation['Strategy_Daily_Return']
    rf_annual = 0.04
    ...
    return df_pnl_deviation, sharpe



#excluding these lines so they don't fire everytime I run the file   
#result = run_strategy_deviation(df_wide, window=30, k_mult=1.5)
#print(result)

#now I want to create the same function but instead of using the deviation of the std deviation, I want to use traditional Bollinger bands to test the difference in performances
def run_strategy_bollinger(df_wide, window, k_mult):

    #create new column for price ratio
    df_price_bollinger = df_wide.with_columns(
    (pl.col('KO US EQUITY') / pl.col('PEP US EQUITY')).alias('KO / PEP Price Ratio'))
    #print(df_price)

    #create new column to determine 'normal' price ratio
    #test with 30day rolling average
    df_ratio_bollinger = df_price_bollinger.with_columns(
        (pl.col('KO / PEP Price Ratio').rolling_mean(window_size=window).alias('Moving 30 Day Price Ratio Avg'),
        pl.col('date').cast(pl.Date)))
    #print(df_ratio)

    #create new column with deviation from avg
    df_error_bollinger = df_ratio_bollinger.with_columns(
        ((pl.col('Moving 30 Day Price Ratio Avg') - pl.col('KO / PEP Price Ratio')).alias('Deviation')))
    #print(df_error)

    #create new column with absolute value 30 day rolling std deviation
    #using this column to create boundaries for the DAILY price ratio
    df_noise_bollinger = df_error_bollinger.with_columns(
        (pl.col('KO / PEP Price Ratio').rolling_std(window_size=window).abs().alias('Moving 30 Day Price Ratio Std Deviation'),
        pl.col('date').cast(pl.Date)))
    #print(df_noise)
    ...
    df_boundary_bollinger = df_noise_bollinger.with_columns(
        ((pl.col('Moving 30 Day Price Ratio Avg') + (pl.col('Moving 30 Day Price Ratio Std Deviation') * k_mult)).alias('Upper Bound')),
        (((pl.col("Moving 30 Day Price Ratio Avg")) - (pl.col('Moving 30 Day Price Ratio Std Deviation') * k_mult)).alias('Lower Bound')),
        pl.col('date').cast(pl.Date))
    #print(df_boundary)
    ...
    #-1 we short the relationship with the expectation that it will fall back to normal
    #1 we buy the relationship with the hope it will rise back to normal
    df_conditional_bollinger = df_boundary_bollinger.with_columns(
        pl.when(pl.col("KO / PEP Price Ratio") > pl.col('Upper Bound')).then(pl.lit(-1))
        .when(pl.col("KO / PEP Price Ratio") < pl.col('Lower Bound')).then(pl.lit(1))
        .otherwise(pl.lit(0))
        .alias("Target Position"))
    ...
    #computing simulate PnL for 2023
    df_pnl_bollinger = df_conditional_bollinger.with_columns(
        (pl.col('KO US EQUITY').pct_change().alias('Ret_KO')),
        (pl.col('PEP US EQUITY').pct_change().alias('Ret_PEP')),
        (pl.col('Target Position').shift(1).alias('Position Held'))
        ).with_columns(
    ...
    #assume Risk Free Rate is ~4%
    daily_returns = df_pnl_bollinger['Strategy_Daily_Return']
    rf_annual = 0.04
    rf_daily = (1 + rf_annual) ** (1/252) - 1
    excess_returns = daily_returns - rf_daily
    ...
    return df_pnl_bollinger, sharpe
...
def evaluate_params(args):
    window, k = args
    try:
        df_result, sharpe_result = run_strategy_bollinger(df_wide, window = int(window), k_mult=float(k))
        return {
            'window': window,
            'k': k,
...
if __name__ == '__main__':
    ...
    #end results are still working as expected which is a good sign
    print(best_window, best_k)
...
for test_window in potential_windows:
    for test_k in k_multipliers:
        df_result, sharpe_result = run_strategy_deviation(df_wide, window=int(test_window), k_mult=float(test_k))
